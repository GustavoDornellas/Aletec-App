...
export default function Login() {
    ...
    return (<div className="min-h-screen bg-surface relative overflow-hidden">
      ...
      <div className="relative min-h-screen flex items-center justify-center p-4 md:p-8">
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-6xl overflow-hidden rounded-[32px] border border-slate-200 bg-white shadow-[0_24px_80px_rgba(15,23,42,0.12)]">
            ...
            <section className="bg-white px-6 py-8 md:px-10 md:py-12">
              <div className="mx-auto flex h-full w-full max-w-md flex-col justify-center">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  {error && (<div className="rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-medium text-red-700">
                      {error}
                    </div>)}

                  <div className="space-y-2">
                    <label className="block text-[11px] font-black uppercase tracking-[0.24em] text-slate-500">E-mail</label>
                    <input type="email" value={username} onChange={(e) => setUsername(e.target.value)} className={`w-full rounded-2xl border px-4 py-3.5 text-sm text-slate-900 outline-none transition-all ${hasCredentialError
            ? 'border-red-300 bg-red-50/70 focus:border-red-300 focus:bg-white focus:ring-4 focus:ring-red-100'
            : 'border-slate-200 bg-slate-50 focus:border-primary/40 focus:bg-white focus:ring-4 focus:ring-primary/10'}`} placeholder="voce@aletec.com.br" aria-invalid={hasCredentialError} required/>
                  </div>

                  <div className="space-y-2">
                    <label className="block text-[11px] font-black uppercase tracking-[0.24em] text-slate-500">Senha</label>
                    <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className={`w-full rounded-2xl border px-4 py-3.5 text-sm text-slate-900 outline-none transition-all ${hasCredentialError
            ? 'border-red-300 bg-red-50/70 focus:border-red-300 focus:bg-white focus:ring-4 focus:ring-red-100'
            : 'border-slate-200 bg-slate-50 focus:border-primary/40 focus:bg-white focus:ring-4 focus:ring-primary/10'}`} placeholder="Digite sua senha" aria-invalid={hasCredentialError} required/>
                  </div>

                  <button type="submit" disabled={isSubmitting} className="flex w-full items-center justify-center gap-3 rounded-2xl bg-primary px-4 py-3.5 text-sm font-black text-on-primary shadow-lg shadow-primary/20 transition-all hover:bg-primary-dim active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-70">
                    {isSubmitting ? (<>
