...
const normalizeCsvValue = (value: string | null | undefined) => (value ?? '').replace(/\r/g, '').trim();
...
const CSV_HEADER_LOOKUP = new Map(
  CSV_FIELDS.flatMap((field) =>
    [field.key, ...field.aliases, field.label].map((alias) => [normalizeCsvHeader(alias), field.key] as const)
  )
);
...
const parseCsvContent = (content: string) => {
  ...
  return rows.map((row, index) => ({
    rowNumber: index + 2,
    entry: headers.reduce<Record<string, string>>((acc, header, headerIndex) => {
...
export default function Inventory() {
  ...
  const filteredUnitsByProduct = useMemo(
    () =>
      Object.fromEntries(
  ...
  const inventoryValueForSale = useMemo(
    () => products.reduce((acc, product) => acc + (product.available || 0) * (product.price || 0), 0),
    [products]
  );
    ...
    try {
      ...
      if (missingHeaders.length > 0) {
        const missingLabels = missingHeaders.map(
          (header) => CSV_FIELDS.find((field) => field.key === header)?.label || header
        );
      ...
      const existingUnits = new Set(
        Object.entries(units).flatMap(([productId, productUnits]) =>
      ...
      showSuccess(
        `Importacao concluida. ${importedProducts} produto(s) criado(s), ${importedUnits} unidade(s) importada(s)` +
    ...
    >
          ...
          <div className="flex w-full md:w-auto flex-col sm:flex-row gap-2">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="bg-white text-slate-700 px-4 py-2.5 rounded-lg font-semibold text-sm flex items-center justify-center gap-2 border border-slate-200 hover:bg-slate-50 transition-all disabled:opacity-60 dark:bg-[#34322f] dark:text-zinc-100 dark:border-[#45413c] dark:hover:bg-[#3c3935]"
              >
              ...
              <button
                ...
                className="bg-white text-slate-700 px-4 py-2.5 rounded-lg font-semibold text-sm flex items-center justify-center gap-2 border border-slate-200 hover:bg-slate-50 transition-all dark:bg-[#34322f] dark:text-zinc-100 dark:border-[#45413c] dark:hover:bg-[#3c3935]"
              >
                <Download size={16} />
                Exportar CSV
              </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        {inventoryStats.map((stat, i) => (
          <div key={i} className="bg-white p-5 md:p-6 rounded-xl border-b-2 border-primary/10 shadow-sm dark:bg-[#34322f] dark:border-blue-500/35">
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 flex flex-col md:flex-row gap-4 items-center dark:bg-[#34322f] dark:border-[#45413c]">
        <div className="relative flex-1 w-full">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-primary/20 transition-all outline-none dark:bg-[#2f2c29] dark:border-[#4a4540] dark:text-blue-100 dark:placeholder:text-blue-200/40"
          />
        </div>
        ...
        <div className="flex gap-2 w-full md:w-auto">
          <div className="relative flex-1 md:w-48">
            ...
            <select
              ...
              className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs font-bold text-slate-600 appearance-none focus:ring-2 focus:ring-primary/20 outline-none dark:bg-[#2f2c29] dark:border-[#4a4540] dark:text-blue-100"
            >
            ...
            return (
              <div key={product.id} className="contents">
                <div
                  ...
                  className="hidden md:grid grid-cols-12 gap-4 px-6 py-5 items-center hover:bg-slate-50/50 transition-colors cursor-pointer group dark:hover:bg-zinc-800/40"
                >
                  ...
                  <div className="col-span-4 flex items-center gap-4">
                    <div
                      className={`w-12 h-12 bg-slate-100 rounded-lg flex-shrink-0 flex items-center justify-center p-2 relative overflow-hidden dark:bg-[#2f2c29] ${
                ...
                <AnimatePresence>
                  {expandedId === product.id && (
                    <motion.div
                      ...
                      className="overflow-hidden bg-slate-50/50 border-y border-slate-100 dark:bg-[#2b2927] dark:border-[#45413c]"
                    >
                      <div className="px-4 md:px-6 py-4 space-y-2">
                        ...
                        {filteredUnits.map((unit) => (
                          <div key={unit.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-4 md:px-10 py-3 bg-white rounded-lg border border-slate-200 shadow-sm gap-2 dark:bg-[#34322f] dark:border-[#45413c]">
                            <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-start">
                              {unit.image && (
                                <div
                                  className="relative w-10 h-10 rounded-md overflow-hidden border border-slate-200 flex-shrink-0 cursor-zoom-in dark:border-zinc-700"
                        ...
                        {filteredUnits.length === 0 && (
                          <p className="text-center py-4 text-xs text-slate-400 dark:text-blue-300/65">Nenhuma unidade encontrada com os filtros atuais.</p>
                        )}
                      </div>
                    </motion.div>
                  )}
