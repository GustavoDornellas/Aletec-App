...
export function handleSupabaseError(error, operationType, path) {
    ...
    const friendlyMessage = normalizedMessage.includes('products_pn_key') ||
        normalizedMessage.includes('products_pn_category_key') ||
        normalizedMessage.includes('duplicate key value')
        ? 'Ja existe um item cadastrado com esse modelo nessa categoria. Use outra categoria ou edite o item existente.'
        : message;
    const errInfo = {
